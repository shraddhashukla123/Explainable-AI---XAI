...
from pandas.api.types import (is_bool_dtype, is_datetime64_any_dtype, 
    is_timedelta64_dtype, is_integer_dtype, is_float_dtype, is_numeric_dtype,
    is_string_dtype,)

def detect_datatypes(df: pd.DataFrame, 
    cat_max_unique: int = 10, cat_ratio_threshold: float = 0.05,) -> pd.DataFrame:
    ...
    for col in df.columns:
        ...
        is_id = bool(re.search(r'(^id$|_id$|^id_|uuid|guid|_code$|^code$|_key$|^key$)', col.lower()))
        ...
        if col.strip().lower().replace(" ", "_") in {"holiday_flag", "year", "yr_week", 
                                                    "period", "qtr",}:
            use_for_model = False

        if col.strip().lower() in {"region", "category_area", "product",}:
            use_for_model = False
    ...
    schema.loc[
        schema["column"].isin(["Period", "Qtr"]),
        ["likely_categorical", "is_high_cardinality", "use_for_model"]
    ] = [True, False, True]

    schema.loc[
        schema["column"].isin(["Period", "Qtr"]),
        "needs_minmax_scaling"
    ] = False
